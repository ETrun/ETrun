// cg_scoreboard -- draw the scoreboard on top of the game screen
#include "cg_local.h"


#define	SCOREBOARD_WIDTH	(31*BIGCHAR_WIDTH)

// Colors
vec4_t clrUiBack = { 0.f, 0.f, 0.f, .5f };
vec4_t clrUiYou = { 0.2f, 0.1f, 0.1f, .5f };

/*
=================
WM_DrawObjectives
=================
*/

#define INFO_PLAYER_WIDTH				64
#define INFO_TEAM_WIDTH					36
#define INFO_SCORE_WIDTH				48
#define INFO_SPEED_WIDTH				36
#define INFO_LATENCY_WIDTH				24
#define INFO_STATE_WIDTH				48

#define INFO_TOTAL_WIDTH				(INFO_PLAYER_WIDTH + INFO_TEAM_WIDTH + INFO_SCORE_WIDTH + INFO_SPEED_WIDTH + INFO_LATENCY_WIDTH + INFO_STATE_WIDTH)

#define INFO_LINE_HEIGHT				10

#define INFO_SPEC_PLAYER_WIDTH			INFO_PLAYER_WIDTH
#define INFO_SPEC_FOLLOWED_PLAYER_WIDTH	INFO_PLAYER_WIDTH
#define INFO_SPEC_SCORE_WIDTH			48
#define INFO_SPEC_LATENCY_WIDTH			24
#define INFO_SPEC_TOTAL_WIDTH			(INFO_SPEC_PLAYER_WIDTH + INFO_SPEC_FOLLOWED_PLAYER_WIDTH + INFO_SPEC_SCORE_WIDTH + INFO_SPEC_LATENCY_WIDTH)

#define NAME_MAX_LENGHT					15

#define GOOD_PING_LIMIT					50
#define	MEDIUM_PING_LIMIT				100

#define NUM_PLAYERS_LAYOUT				8// Nico, if there are more than 8 players, split the scoreboard in two columns	

/* ETrun jump n newlines
 *
 * @author Nico
 */
static void WM_ETrun_newlines(int n, int *y, int newlineSize) {
	*y += newlineSize * n;
}

/* ETrun draw text utility
 *
 * @author Nico
 */
static void WM_ETrun_print(const char *s, fontInfo_t *font, float scale, int x, int y, qboolean shadowed) {
	vec4_t clrUiWhite = { 1.0f, 1.0f, 1.0f, 0.8f };

	if (shadowed == qtrue) {
		CG_Text_Paint_Ext(x, y, scale, scale, clrUiWhite, s, 0, 0, ITEM_TEXTSTYLE_SHADOWED, font);
	} else {
		CG_Text_Paint_Ext(x, y, scale, scale, clrUiWhite, s, 0, 0, 0, font);
	}
}

/* ETrun draw text utility
 *
 * @author Nico
 */
static void WM_ETrun_center_print(const char *s, fontInfo_t *font, float scale, int y, qboolean shadowed) {
	WM_ETrun_print(s, font, scale, SCREEN_WIDTH / 2 - CG_Text_Width_Ext(s, scale, 0, font) / 2, y, shadowed);
}

/* Insert sort
 *
 * @author Nico
 */
static void insert_sort(s_timerunScores *tab, int size) {
   int i;
   int j;

   for (i = 1; i < size; ++i) {
       s_timerunScores elem = tab[i];
	   for (j = i; j > 0 && tab[j - 1].timerunBestTime > elem.timerunBestTime; j--)
           tab[j] = tab[j - 1];
       tab[j] = elem;
   }
}

/* New ETrun color ping function
 *
 * @author Nico
 */
static char *WM_ETrun_coloredPing(int ping) {
	char *s;

	if (ping <= GOOD_PING_LIMIT) {
		s = va("^2%i", ping);
	} else if (ping <= MEDIUM_PING_LIMIT) {
		s = va("^3%i", ping);
	} else {
		s = va("^1%i", ping);
	}

	return s;
}

/* New ETrun draw header function
 *
 * @author Nico
 */
static void WM_ETrun_DrawHeader(int *x, int *y, int width, float fade, fontInfo_t *font) {
	const char *s;

	// ETrun x.x
	s = GAME_VERSION_COLORED " " MOD_VERSION_COLORED " " MOD_STATUS_COLORED;
	WM_ETrun_center_print(s, font, 0.3f, *y, qtrue);

	WM_ETrun_newlines(3, y, SMALLCHAR_HEIGHT);

	// Map
	s = va(CG_TranslateString(va("Map: ^w%s", cgs.rawmapname)));
	WM_ETrun_center_print(s, font, 0.25f, *y, qtrue);

	WM_ETrun_newlines(3, y, SMALLCHAR_HEIGHT);

	// www.timeruns.net
	s = SHORT_MOD_URL;
	WM_ETrun_center_print(s, font, 0.25f, *y, qtrue);
	
	WM_ETrun_newlines(4, y, SMALLCHAR_HEIGHT);
}

/* New ETrun draw players function
 *
 * @author Nico
 */
static void WM_ETrun_DrawPlayers(int *x, int *y, int width, float fade, fontInfo_t *font, s_timerunScores *orderedScores) {
	char *s = NULL;
	int tempx = 0;
	float fontsize = 0.14f;
	int i = 0;
	int mil, min, sec;

	// Draw "Players"
	WM_ETrun_center_print("Players", font, 0.25f, *y, qtrue);

	WM_ETrun_newlines(1, y, SMALLCHAR_HEIGHT);

	// Draw player info headings background
	
	CG_FillRect(*x, *y, INFO_TOTAL_WIDTH, INFO_LINE_HEIGHT, clrUiBack);
	CG_DrawRect_FixedBorder(*x, *y, INFO_TOTAL_WIDTH, INFO_LINE_HEIGHT, 1, colorBlack);
	*y += INFO_LINE_HEIGHT - 2;

	// Draw player info headings
	tempx = *x;
	WM_ETrun_print("Name", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_PLAYER_WIDTH;

	WM_ETrun_print("Team", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_TEAM_WIDTH;

	WM_ETrun_print("Time", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_SCORE_WIDTH;

	WM_ETrun_print("Speed", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_SPEED_WIDTH;

	WM_ETrun_print("Ping", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_LATENCY_WIDTH;

	WM_ETrun_print("State", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_STATE_WIDTH;

	for (i = 0; i < cg.numScores; ++i) {

		// Ignore spectators
		if (orderedScores[i].team == TEAM_SPECTATOR) {
			continue;
		}

		tempx = *x;
		*y += 4;

		// Draw background (highlight your line)
		CG_FillRect(*x, *y, INFO_TOTAL_WIDTH, INFO_LINE_HEIGHT, orderedScores[i].clientNum == cg.snap->ps.clientNum ? clrUiYou : clrUiBack);
		CG_DrawRect_FixedBorder(*x, *y, INFO_TOTAL_WIDTH, INFO_LINE_HEIGHT, 1, colorBlack);
		*y += INFO_LINE_HEIGHT - 2;

		// Nico, draw player name
		WM_ETrun_print(orderedScores[i].name, font, fontsize, tempx, *y, qtrue);
		tempx += INFO_PLAYER_WIDTH;
		
		// Nico, draw team
		if (orderedScores[i].team == TEAM_AXIS) {
			s = "^1Axis";
		} else {
			s = "^4Allies";
		}
		WM_ETrun_print(s, font, fontsize, tempx, *y, qtrue);
		tempx += INFO_TEAM_WIDTH;

		// Nico, show best time
		if (orderedScores[i].timerunBestTime) {
			mil = orderedScores[i].timerunBestTime;
			min = mil / 60000;
			mil -= min * 60000;
			sec = mil / 1000;
			mil -= sec * 1000;

			s = va("^7%02d:%02d.%03d", min, sec, mil);
		} else {
			s = "-";
		}
		WM_ETrun_print(s, font, fontsize, tempx, *y, qtrue);
		tempx += INFO_SCORE_WIDTH;

		// Nico, draw best speed
		WM_ETrun_print(va("%d", orderedScores[i].timerunBestSpeed), font, fontsize, tempx, *y, qtrue);
		tempx += INFO_SPEED_WIDTH;

		// Nico, draw ping
		WM_ETrun_print(WM_ETrun_coloredPing(orderedScores[i].ping), font, fontsize, tempx, *y, qtrue);
		tempx += INFO_LATENCY_WIDTH;

		// Nico, draw timerun status
		if (orderedScores[i].timerunStatus == 1) {
			s = "^2Running";
		} else {
			s = "^3Not running";
		}
		WM_ETrun_print(s, font, fontsize, tempx, *y, qtrue);
		tempx += INFO_STATE_WIDTH;
	}
	WM_ETrun_newlines(2, y, SMALLCHAR_HEIGHT);
}

/* New ETrun draw spectators function
 *
 * @author Nico
 */
static void WM_ETrun_DrawSpectators(int *x, int *y, int width, float fade, fontInfo_t *font, s_timerunScores *orderedScores) {
	char *s = NULL;
	int tempx = 0;
	float fontsize = 0.14f;
	int i = 0;
	int mil, min, sec;

	// Draw "Spectators"
	WM_ETrun_center_print("Spectators", font, 0.25f, *y, qtrue);

	WM_ETrun_newlines(1, y, SMALLCHAR_HEIGHT);

	// Draw player info headings background
	CG_FillRect(*x, *y, INFO_SPEC_TOTAL_WIDTH, INFO_LINE_HEIGHT, clrUiBack);
	CG_DrawRect_FixedBorder(*x, *y, INFO_SPEC_TOTAL_WIDTH, INFO_LINE_HEIGHT, 1, colorBlack);
	*y += INFO_LINE_HEIGHT - 2;

	// Draw spec info headings
	tempx = *x;
	WM_ETrun_print("Name", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_SPEC_PLAYER_WIDTH;

	WM_ETrun_print("Following", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_SPEC_FOLLOWED_PLAYER_WIDTH;

	WM_ETrun_print("Time", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_SPEC_SCORE_WIDTH;

	WM_ETrun_print("Ping", font, fontsize, tempx, *y, qtrue);
	tempx += INFO_SPEC_LATENCY_WIDTH;

	for (i = 0; i < cg.numScores; ++i) {

		// Ignore non-spectators
		if (orderedScores[i].team != TEAM_SPECTATOR) {
			continue;
		}

		tempx = *x;
		*y += 4;

		// Draw background (highlight your line)
		CG_FillRect(*x, *y, INFO_SPEC_TOTAL_WIDTH, INFO_LINE_HEIGHT, orderedScores[i].clientNum == cg.snap->ps.clientNum ? clrUiYou : clrUiBack);
		CG_DrawRect_FixedBorder(*x, *y, INFO_SPEC_TOTAL_WIDTH, INFO_LINE_HEIGHT, 1, colorBlack);
		*y += INFO_LINE_HEIGHT - 2;

		// Nico, draw player name
		WM_ETrun_print(orderedScores[i].name, font, fontsize, tempx, *y, qtrue);
		tempx += INFO_SPEC_PLAYER_WIDTH;

		// Nico, draw followed client name
		if (orderedScores[i].clientNum != orderedScores[i].followedClient) {
			s = orderedScores[i].followedClientName;	
		}  else {
			s = "-";
		}
		WM_ETrun_print(s, font, fontsize, tempx, *y, qtrue);	
		tempx += INFO_SPEC_FOLLOWED_PLAYER_WIDTH;

		// Nico, show best time
		if (orderedScores[i].timerunBestTime) {
			mil = orderedScores[i].timerunBestTime;
			min = mil / 60000;
			mil -= min * 60000;
			sec = mil / 1000;
			mil -= sec * 1000;

			s = va("^7%02d:%02d.%03d", min, sec, mil);
		} else {
			s = "-";
		}
		WM_ETrun_print(s, font, fontsize, tempx, *y, qtrue);
		tempx += INFO_SPEC_SCORE_WIDTH;

		// Nico, draw ping
		WM_ETrun_print(WM_ETrun_coloredPing(orderedScores[i].ping), font, fontsize, tempx, *y, qtrue);
		tempx += INFO_SPEC_LATENCY_WIDTH;
	}
}

/*
=================
CG_DrawScoreboard

Draw the normal in-game scoreboard
=================
*/
qboolean CG_DrawScoreboard(void) {
	int		x = 0;
	int		y = 0;
	float	fade;
	float	*fadeColor;
	s_timerunScores orderedScores[MAX_CLIENTS];
	int i = 0;
	int j = 0;
	
	// don't draw anything if the menu or console is up
	if (cg_paused.integer) {
		return qfalse;
	}

	// don't draw scoreboard during death while warmup up
	// OSP - also for pesky scoreboards in demos
	if (!cg.showScores) {
		return qfalse;
	}

	// don't draw if in cameramode
	if (cg.cameraMode) {
		return qtrue;
	}

	if (cg.showScores) {
		fade = 1.0;
		fadeColor = colorWhite;
	} else {
		fadeColor = CG_FadeColor(cg.scoreFadeTime, FADE_TIME);
 		fade = fadeColor[3];
	}
	
	// Nico, update ordered scores

	// Fill players array
	for (i = 0; i < cg.numScores; ++i) {
		orderedScores[i].scoreId = i;
		orderedScores[i].clientNum = cg.scores[i].client;
		Q_strncpyz(orderedScores[i].name, cgs.clientinfo[cg.scores[i].client].name, NAME_MAX_LENGHT);
		orderedScores[i].team = cg.scores[i].team;
		orderedScores[i].timerunBestTime = cg.scores[i].timerunBestTime;// Best time
		orderedScores[i].timerunBestSpeed = cg.scores[i].timerunBestSpeed;// Best speed
		orderedScores[i].timerunStatus = cg.scores[i].timerunStatus;// Timerun status
		orderedScores[i].ping = cg.scores[i].ping;
		orderedScores[i].followedClient = cg.scores[i].followedClient;// Followed client
		Q_strncpyz(orderedScores[i].followedClientName, cgs.clientinfo[cg.scores[i].followedClient].name, NAME_MAX_LENGHT);// Followed client name
	}

	// Sort it
	insert_sort(orderedScores, cg.numScores);

	// Put 00:00.00 at end of array
	i = 0;
	while ((i < cg.numScores) && (orderedScores[i].timerunBestTime == 0)) {
		for (j = i; j < cg.numScores; ++j) {
			if (orderedScores[j].timerunBestTime != 0) {
				s_timerunScores temp = orderedScores[j];
				orderedScores[j] = orderedScores[i];
				orderedScores[i] = temp;
				break;
			}
		}
		i++;
	}

	// Count team players
	cg.teamPlayers[TEAM_AXIS] = 0;
	cg.teamPlayers[TEAM_ALLIES] = 0;
	cg.teamPlayers[TEAM_SPECTATOR] = 0;
	for (i = 0; i < cg.numScores; ++i) {
		cg.teamPlayers[cg.scores[i].team]++;
	}

	if (cg.numScores < NUM_PLAYERS_LAYOUT) {
		// Nico, single olumn scoreboard
		x = SCREEN_WIDTH / 2;// Nico, center horizontally
		y = 50;// Start drawing from y = 50

		// Nico, draw scoreboard header
		WM_ETrun_DrawHeader(&x, &y, SCREEN_WIDTH - 2 * x, fade, &cgs.media.limboFont1);

		// Nico, draw players, if any
		x = (SCREEN_WIDTH - INFO_TOTAL_WIDTH) / 2;// Nico, center horizontally
		if (cg.teamPlayers[TEAM_ALLIES] != 0 || cg.teamPlayers[TEAM_AXIS] != 0 ) {
			WM_ETrun_DrawPlayers(&x, &y, SCREEN_WIDTH - 2 * x, fade, &cgs.media.limboFont1, orderedScores);
		}

		// Nico, draw spectators, if any
		x = (SCREEN_WIDTH - INFO_SPEC_TOTAL_WIDTH) / 2;// Nico, center horizontally
		if (cg.teamPlayers[TEAM_SPECTATOR] != 0) {
			WM_ETrun_DrawSpectators(&x, &y, SCREEN_WIDTH - 2 * x, fade, &cgs.media.limboFont1, orderedScores);
		}
	} else {
		// Nico, 2-columns scoreboard

	}


	
	return qtrue;
}
